...
- exam and cv: use a paged layout (perfectly fit to page width and height, with margin and page number, header and footer), let user to select page size (A4, letter, etc). When using the preview to export pdf file, make sure what is seen in preview is exactly what is get in pdf. Use nyml plugin in the source markdown, which will be parsed into json object in a div element, the editor will use the json object to create title section/page, add page break, render elements with special styles such as questions, cv entries. 
- slide: use a paged layout (perfectly fit to screen sizes, let user to select respect ratio, such as 16:9, 4:3, etc). When changing the size of the preview, all contents scale proportionally. 


## Chemical structure drawer

- Use Ketcher (an open source js chemical structure drawer) as chemical structure editor, associate with extension name .mol. 
- Remove the default open and save function from ketcher, add save button and auto save function, as well as history version control. 
- Share raw: not literaly share the raw .mol file, but share the rendered svg image of the structure or chemical reaction scheme with a container just enough to wrap the content. 
- Share rendered: share the rendered svg image in a html page, similar as other share rendered page, provide a download link to download the raw .mol file. 